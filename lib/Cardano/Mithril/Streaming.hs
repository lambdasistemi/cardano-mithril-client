{- |
Module      : Cardano.Mithril.Streaming
Description : Stream UTxOs to bulk import

This module provides functionality to stream UTxOs from Mithril extraction
into a database. It supports:
...
The streaming approach follows a memory-efficient pattern for processing
large UTxO sets.
-}
module Cardano.Mithril.Streaming
    ( -- * Streaming
...
-- | Configuration for streaming to database
...
streamToCSMT
    ...
    -> IO Word64
streamToCSMT tracer config insertKV stream = do
            ...
            Right ((keyBs, valueBs), rest) -> do
                ...
                case convertToCBOR keyBs valueBs of
                    ...
                    Just (cborKey, cborValue) -> do
                        -- Insert CBOR-encoded bytes into database
                        insertKV cborKey cborValue
